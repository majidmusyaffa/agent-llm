...
from database_tools import init_database, get_database_info, text_to_sql, get_database_metadata
import logging

logging.basicConfig(level=logging.INFO)

st.title("Agentic AI SQL Query dengan File Upload")

# Upload file dulu
uploaded_file = st.file_uploader("Upload CSV atau XLSX", type=["csv", "xlsx"])
if uploaded_file:
    st.session_state["db_msg"] = init_database(uploaded_file, uploaded_file.name)
    st.success(st.session_state["db_msg"])

    schema_str, df_sample = get_database_metadata()
    st.text(schema_str)        # schema sebagai teks
    st.dataframe(df_sample)    # sample data tampil sebagai tabel interaktif
...
def execute_sql(sql_query: str):
    ...
    """
    logging.info(f"[execute_sql] Dijalankan dengan query:\n{sql_query}")
...
def get_schema_info() -> str:
    """Return schema + sample data ringkas untuk membantu query SQL."""
    logging.info("[get_schema_info] Dijalankan")

    info = get_database_info()
    if "error" in info:
        return f"ERROR: {info['error']}"

    schema_str = "\n".join([f"- {col} ({dtype})" for col, dtype in info["schema"].items()])
    sample_str = "\n".join([str(row) for row in info["sample_data"][:3]])
...
prompt = st.chat_input("Tanya apapun terkait data yang diinput..")
